...
rats_verifier_err_t ecdsa_verify_evidence(rats_verifier_ctx_t *ctx, sgx_quote3_t *pquote,
					  uint32_t quote_size,
					  attestation_endorsement_t *endorsements)
{
	...
	if (is_sgx_ecdsa_qve) {
		...
		if (sgx_self_target_ret != SGX_SUCCESS) {
			RATS_ERR("failed to get self target info sgx_self_target_ret. %04x\n",
				 sgx_self_target_ret);
...
rats_verifier_err_t ecdsa_verify_evidence(sgx_quote3_t *pquote, uint32_t quote_size,
					  attestation_endorsement_t *endorsements)
{
			  ...
			  __attribute__((unused)) size_t *claims_length)
{
	...
	if (err != RATS_VERIFIER_ERR_NONE)
		RATS_ERR("failed to verify ecdsa, %04x\n", err);
#else
