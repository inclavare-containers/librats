...
rats_attester_err_t sev_snp_get_vcek_der(const uint8_t *chip_id, size_t chip_id_size, const snp_tcb_version_t *tcb,snp_attestation_evidence_t *snp_report){
	...
	count += snprintf((char *)&url[count], sizeof(url) - count,
					     ...
					     uint32_t hash_len)
{
	...
	rats_attester_err_t err = sev_snp_get_vcek_der(report.chip_id,sizeof(report.chip_id),&report.platform_version,snp_report);
